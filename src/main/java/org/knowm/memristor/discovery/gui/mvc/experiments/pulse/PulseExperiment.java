...
public class PulseExperiment extends Experiment {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		case Model.EVENT_WAVEFORM_UPDATE:
			...
			resultController.updateWaveformChart(controlModel.getWaveformTimeData(),
					controlModel.getWaveformAmplitudeData(), controlModel.getAmplitude(), controlModel.getPulseWidth());
	...
	private class CaptureWorker extends SwingWorker<Boolean, double[][]> {
		...
		protected Boolean doInBackground() throws Exception {
			...
			dwfProxy.getDwf().startAnalogCaptureBothChannelsTriggerOnWaveformGenerator(DWF.WAVEFORM_CHANNEL_1,
					sampleFrequency, bufferSize, isScale2V);
			...
			if (boardVersion == 2) {
				customWaveform = WaveformUtils.generateCustomPulse(controlModel.getWaveform(),
						-controlModel.getAmplitude(), controlModel.getPulseWidth(), controlModel.getDutyCycle());
			} else {
				customWaveform = WaveformUtils.generateCustomPulse(controlModel.getWaveform(),
						controlModel.getAmplitude(), controlModel.getPulseWidth(), controlModel.getDutyCycle());
			}
...
//      System.out.println(Arrays.toString(customWaveform));
			dwfProxy.getDwf().startCustomPulseTrain(DWF.WAVEFORM_CHANNEL_1, controlModel.getCalculatedFrequency(), 0,
					controlModel.getPulseNumber(), customWaveform);

			// ////////////////////////////////

			// Read In Data
			boolean success = dwfProxy.capturePulseData(controlModel.getCalculatedFrequency(),
			...
			if (boardVersion == 2) {
				...
				for (int i = 0; i < bufferLength; i++) {
					current[i] = (V1Trimmed[i] - V2Trimmed[i]) / controlModel.getSeriesResistance()
				...
				publish(new double[][] { timeData, V1Trimmed, V2Trimmed, VMemristor, current, conductance, null });
			} else {
				bufferLength = V1Trimmed.length;
				...
				double[] current = new double[bufferLength];
				for (int i = 0; i < bufferLength; i++) {
					current[i] = V2Trimmed[i] / controlModel.getSeriesResistance()
							* PulsePreferences.CURRENT_UNIT.getDivisor();
				}
			...
			while (!isCancelled()) {
				...
				dwfProxy.getDwf().startAnalogCaptureBothChannelsTriggerOnWaveformGenerator(DWF.WAVEFORM_CHANNEL_1,
						sampleFrequency, bufferSize, true);
				dwfProxy.waitUntilArmed();
				...
				customWaveform = WaveformUtils.generateCustomWaveform(Waveform.Square, readPulseAmplitude, f);
				...
				if (!success) {
					...
					double[] conductanceAve = new double[] {
		...
		protected void process(List<double[][]> chunks) {
			...
			if (newestChunk[6] == null) {
				...
				resultController.updateCaptureChartData(newestChunk[0], newestChunk[1], newestChunk[2], newestChunk[3],
						controlModel.getPulseWidth(), controlModel.getAmplitude());
				resultController.updateIVChartData(newestChunk[0], newestChunk[4], controlModel.getPulseWidth(),
				...
				resultController.updateReadPulseCaptureChartData(newestChunk[0], newestChunk[1], newestChunk[2],
						newestChunk[3], controlModel.getPulseWidth(), controlModel.getAmplitude());
				resultController.repaintReadPulseCaptureChart();

				// update G chart
				controlModel.setLastG(newestChunk[6][0]);
				resultController.updateGChartData(controlModel.getLastG(), controlModel.getLastR(),
						controlModel.getLastRAsString());
				resultController.repaintGChart();

				controlModel.updateEnergyData();
				controlPanel.updateEnergyGUI(controlModel.getAmplitude(), controlModel.getAppliedCurrent(),
	...
	private void initResistanceComputer() {
		...
		double pulseAmp = ad2BugCalibrationValues.getReadPulseOffset()
		...
		if (rcComputer != null) {
			...
			if (Math.abs(dpA) < .05) {
				...
				LOGGER.warn("Measured pulse amplitude has drifted by " + dpA
						+ ". RC Computer re-initialized. If this is frequent there is a noise problem.");
			}
		}

		long t = System.currentTimeMillis();
		this.rcComputer = new RC_ResistanceComputer(boardVersion, readPulseAve.getValue(), pulseWidth,
				controlModel.getSeriesResistance(), ad2BugCalibrationValues.getReadPulseZeroOffset(),
				ad2BugCalibrationValues.getReadPulseInitialVoltage());

		LOGGER.info("RC_Resistance Computer was initialized in " + (System.currentTimeMillis() - t) + " ms");
	}
	...
	private void updateReadPulseVoltageAndOffsets(double[] v2, double[] v1) {
		...
		try {
			...
			double[] xvi = Arrays.copyOfRange(v1, 3097, 4097); // take from the end because the start can get clipped
																// for big pulses.
			...
			getControlModel().swingPropertyChangeSupport.firePropertyChange(Model.EVENT_NEW_CONSOLE_LOG, null,
					"Read Pulse Zero Offset: " + ad2BugCalibrationValues.getReadPulseZeroOffset());
