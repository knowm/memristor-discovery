...
public class PulseExperiment extends Experiment {
	...
	private float readPulseCalibration = 0.0f;// calibration may not work. This adjusts the readpulse magnitude so that
												// it equals .1V.
	...
	private class CaptureWorker extends SwingWorker<Boolean, double[][]> {
		...
		protected Boolean doInBackground() throws Exception {
			...
			while (!isCancelled()) {
				...
				dwfProxy.getDwf().startAnalogCaptureBothChannelsTriggerOnWaveformGenerator(DWF.WAVEFORM_CHANNEL_1,
						sampleFrequency, samplesPerPulse, true);
				dwfProxy.waitUntilArmed();
				...
				customWaveform = WaveformUtils.generateCustomWaveform(Waveform.Square,
						controlModel.getReadPulseAmplitude() + readPulseCalibration, f);
				...
				if (!success) {
					...
					if (boardVersion == 2) {
						publish(new double[][] { timeData, V1Trimmed, V2Trimmed, VMemristor, null, null,
								conductanceAve });
					} else {
						publish(new double[][] { timeData, V1Trimmed, V2Trimmed, VMemristor, null, null,
								conductanceAve });
