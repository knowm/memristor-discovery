...
/**
 * used to measure memristor resistance at low currents, when pulse capture
 * results in capacitive charge/discharge instead of steady-state voltage
 * divider. Given board series resistor and parasitic capacitance, will return
 * the estimated resistance given the read square pulse trace.
 *
 * @author alexnugent
 */
public class RC_ResistanceComputer {
	...
	public RC_ResistanceComputer(int boardVersion, double readPulseAmplitude, double readPulseWidth,
			double seriesResistance, double parasiticCapacitance) {
	...
	public void loadTrace() {
		...
		TransientConfig transientConfig = new TransientConfig("" + readPulseWidth, "" + simStepSize,
				new DC("V1", readPulseAmplitude));
