 ...
 *
 * <ul>
 *   <li>Provision of standard template handling for events <strong>with responses</strong>
 * </ul>
 *
 * <p>Template keys are:<br>
 *
 * <ul>
 *   <li><code>H</code>: The generic handler class (e.g. {@link
 ...
 *   <li><code>R</code>: The generic response class (e.g. {@link
 *       org.multibit.platform.listener.GenericQuitResponse}
 * </ul>
 *
 * @since 0.3.0
 */
public abstract class BaseMacResponseInvocationHandler<
        H extends GenericHandler, E extends GenericEvent, R extends GenericEvent>
  ...
  BaseMacResponseInvocationHandler(
      H genericHandler, Class<E> genericEventClass, Class<R> genericResponseClass) {
  ...
  public Object invoke(Object object, Method nativeMethod, Object[] objects) throws Throwable {
    ...
    log.debug(
        "Invoked response method. NativeMethod={}, method args length={}",
        nativeMethod.getName(),
        objects.length);

    // Create a bi-directional generic event using based on 2 parameters (the native event, the
    // native response)
    ...
    try {
      ...
      // Access the equivalent method on the generic handler (e.g.
      // handleQuitRequestWith(GenericQuitEvent event, GenericQuitResponse resonse))
      Method method =
          genericHandler
              .getClass()
              .getMethod(
                  nativeMethod.getName(), new Class[] {genericEventClass, genericResponseClass});
  ...
  private <R> R createGenericResponse(final Object nativeResponse) {
    log.debug(
        "Building response invocation handler. Native {} -> {}",
        nativeResponse.getClass().getSimpleName(),
        genericResponseClass.getSimpleName());
    // The invocation handler manages all method calls against the proxy
    // Relies on the proxy having the same method signatures
    InvocationHandler invocationHandler =
        new InvocationHandler() {
          ...
          public Object invoke(Object o, Method method, Object[] objects) throws Throwable {
            Method nativeMethod =
                nativeResponse.getClass().getMethod(method.getName(), method.getParameterTypes());
            log.debug(
                "Invoking method {}.{}",
                nativeResponse.getClass().getSimpleName(),
                method.getName());
    ...
    return (R)
        Proxy.newProxyInstance(
            getClass().getClassLoader(), new Class[] {genericResponseClass}, invocationHandler);
  }
}
