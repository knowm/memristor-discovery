...
/**
 * A paint class that creates conical gradients around a given center point It could be used in the
 * same way as LinearGradientPaint and RadialGradientPaint and follows the same syntax. You could
 * use floats from 0.0 to 1.0 for the fractions which is standard but it's also possible to use
 * angles from 0.0 to 360 degrees which is most of the times much easier to handle. Gradients always
 * start at the top with a clockwise direction and you could rotate the gradient around the center
 * by given offset. The offset could also be defined from -0.5 to +0.5 or -180 to +180 degrees. If
 * you would like to use degrees instead of values from 0 to 1 you have to use the full constructor
 * and set the USE_DEGREES variable to true
 *
 * @author hansolo
 * @version 1.0
 */
public final class ConicalGradientPaint implements java.awt.Paint {
  ...
  public ConicalGradientPaint(
      ...
      final java.awt.Color[] GIVEN_COLORS)
      throws IllegalArgumentException {
    this(false, CENTER, 0.0f, GIVEN_FRACTIONS, GIVEN_COLORS);
  }

  /**
   * Enhanced constructor which takes the FRACTIONS in degress from 0.0f to 360.0f and also an
   * GIVEN_OFFSET in degrees around the rotation CENTER
   *
   * @param USE_DEGREES
   * @param CENTER
   * @param GIVEN_OFFSET
  ...
  public ConicalGradientPaint(
    ...
    final java.util.ArrayList<Float> FRACTION_LIST =
        new java.util.ArrayList<Float>(GIVEN_FRACTIONS.length);
    final float OFFSET;
    if (USE_DEGREES) {
    ...
    // Adjust fractions and colors array in the case where startvalue != 0.0f and/or endvalue !=
    // 1.0f
    final java.util.List<java.awt.Color> COLOR_LIST =
        new java.util.ArrayList<java.awt.Color>(GIVEN_COLORS.length);
    ...
    final java.util.Map<Float, java.awt.Color> FRACTION_COLORS =
        recalculate(FRACTION_LIST, COLOR_LIST, OFFSET);
    ...
    final java.util.SortedSet<Float> SORTED_FRACTIONS =
        new java.util.TreeSet<Float>(FRACTION_COLORS.keySet());
    ...
    for (int i = 0; i < (COLORS.length - 1); i++) {
      RED_STEP_LOOKUP[i] =
          ((COLORS[i + 1].getRed() - COLORS[i].getRed()) * INT_TO_FLOAT_CONST)
              / (FRACTION_ANGLES[i + 1] - FRACTION_ANGLES[i]);
      GREEN_STEP_LOOKUP[i] =
          ((COLORS[i + 1].getGreen() - COLORS[i].getGreen()) * INT_TO_FLOAT_CONST)
              / (FRACTION_ANGLES[i + 1] - FRACTION_ANGLES[i]);
      BLUE_STEP_LOOKUP[i] =
          ((COLORS[i + 1].getBlue() - COLORS[i].getBlue()) * INT_TO_FLOAT_CONST)
              / (FRACTION_ANGLES[i + 1] - FRACTION_ANGLES[i]);
      ALPHA_STEP_LOOKUP[i] =
          ((COLORS[i + 1].getAlpha() - COLORS[i].getAlpha()) * INT_TO_FLOAT_CONST)
              / (FRACTION_ANGLES[i + 1] - FRACTION_ANGLES[i]);
    }
  }

  /**
   * Recalculates the fractions in the FRACTION_LIST and their associated colors in the COLOR_LIST
   * with a given OFFSET. Because the conical gradients always starts with 0 at the top and
   * clockwise direction you could rotate the defined conical gradient from -180 to 180 degrees
   * which equals values from -0.5 to +0.5
   *
   * @param FRACTION_LIST
   * @param COLOR_LIST
   * @param OFFSET
  ...
  private java.util.HashMap<Float, java.awt.Color> recalculate(
    ...
    // Recalculate the fractions and colors with the given offset
    final int MAX_FRACTIONS = FRACTION_LIST.size();
    final java.util.HashMap<Float, java.awt.Color> FRACTION_COLORS =
        new java.util.HashMap<Float, java.awt.Color>(MAX_FRACTIONS);
    for (int i = 0; i < MAX_FRACTIONS; i++) {
      ...
      if (TMP_FRACTION <= 0) {
        ...
        if (NEXT_FRACTION > 0) {
          final java.awt.Color NEW_FRACTION_COLOR =
  ...
  /**
   * With the START_COLOR at the beginning and the DESTINATION_COLOR at the end of the given RANGE
   * the method will calculate and return the color that equals the given VALUE. e.g. a START_COLOR
   * of BLACK (R:0, G:0, B:0, A:255) and a DESTINATION_COLOR of WHITE(R:255, G:255, B:255, A:255)
   * with a given RANGE of 100 and a given VALUE of 50 will return the color that is exactly in the
   * middle of the gradient between black and white which is gray(R:128, G:128, B:128, A:255) So
   * this method is really useful to calculate colors in gradients between two given colors.
   *
   * @param START_COLOR
  ...
  public java.awt.Color getColorFromFraction(
      ...
      final java.awt.Color DESTINATION_COLOR,
      final int RANGE,
      final int VALUE) {
    ...
    return new java.awt.Color(
        SOURCE_RED + RED_FRACTION * VALUE,
        SOURCE_GREEN + GREEN_FRACTION * VALUE,
        SOURCE_BLUE + BLUE_FRACTION * VALUE,
        SOURCE_ALPHA + ALPHA_FRACTION * VALUE);
  }
  ...
  private final class ConicalGradientPaintContext implements java.awt.PaintContext {
    ...
    public java.awt.image.Raster getRaster(
        final int X, final int Y, final int TILE_WIDTH, final int TILE_HEIGHT) {
      ...
      final java.awt.image.WritableRaster RASTER =
          getColorModel().createCompatibleWritableRaster(TILE_WIDTH, TILE_HEIGHT);
        ...
        for (int px = 0; px < TILE_WIDTH; px++) {
          ...
          for (int i = 0; i < (MAX - 1); i++) {
            if ((angle >= FRACTION_ANGLES[i])) {
              currentRed =
                  COLORS[i].getRed() * INT_TO_FLOAT_CONST
                      + (angle - FRACTION_ANGLES[i]) * RED_STEP_LOOKUP[i];
              currentGreen =
                  COLORS[i].getGreen() * INT_TO_FLOAT_CONST
                      + (angle - FRACTION_ANGLES[i]) * GREEN_STEP_LOOKUP[i];
              currentBlue =
                  COLORS[i].getBlue() * INT_TO_FLOAT_CONST
                      + (angle - FRACTION_ANGLES[i]) * BLUE_STEP_LOOKUP[i];
              currentAlpha =
                  COLORS[i].getAlpha() * INT_TO_FLOAT_CONST
